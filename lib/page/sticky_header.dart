...
class StickyHeaderRoute extends StatefulWidget {
  @override
  StickyHeaderRouteState createState() {
    return new StickyHeaderRouteState();
  }
}

class StickyHeaderRouteState extends State<StickyHeaderRoute> {
  //
...
//    print(json.encode(list).toString());
    //根据A-Z排序
    sortListBySuspensionTag(_contacts);

    print(json.encode(list).toString());
  }

  static void sortListBySuspensionTag(List<ContactInfo> list) {
    if (list == null || list.isEmpty) return;
    print(json.encode(list).toString());
    list.sort((a, b) {
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: <Widget>[
        Container(
          padding: hasHead ? EdgeInsets.only(top: 100) : EdgeInsets.only(),
          child: _listView(),
        ),
        _headView(),
      ],
